...
#include <sys/socket.h>     /* accept, bind, listen, setsockopt, shutdown, */
                            /* socket */
...
int main(void)
{
    ...
    sa.sin_port         = htons(SERVER_PORT);   /* 書き換えた前後の値を確認する */
    sa.sin_addr.s_addr  = htonl(INADDR_ANY);    /* 書き換えた前後の値を確認する */
